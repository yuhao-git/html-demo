  ...
  <ta-border-layout :showBorder="false"
    ...
    <div slot="header">
      ...
      <div class="tree-right">
        <a href="#"
           @click="showChooseFolderModal"
           title="添加任务"
           v-if="showDataValidation !== 2"><img src="./icon/add1.png"
               id="img1"
               class="aimg" /></a>
        <a href="#"
           @click="refresh"
           title="刷新"
           v-if="showDataValidation !== 2"><img src="./icon/refresh1.png"
               id="img2"
               class="aimg"></a>
        <a href="#"
           @click="myMission"
           v-if="showDataValidation !== 2 && isMy"><img src="./icon/my01.png"
               class="aimg"
               title="我的任务" /></a>
        <a href="#"
           @click="myMission"
           v-if="showDataValidation !== 2 && !isMy"><img src="./icon/my02.png"
               class="aimg"
               title="我的任务" /></a>
        <a @click="toBatchGroup"
           v-if="showDataValidation === 2"><ta-icon type="rollback"
    ...
    <div>
      <clone-mission @showMission="getShowMission"
                     ref="cloneMission"
                     @refresh="refresh"
                     :jobId="jobId"
                     :showDataValidation="showDataValidation"></clone-mission>
      <ta-e-tree :props="props"
        ...
        <span class="custom-tree-node"
              ...
              v-on:mouseenter="treeMouseEnter($event.target, node)"
              v-on:mouseleave="treeMouseLeave($event.target)">
          ...
          <span class="tree-operate">
            <span v-if="data.id !== '2' && data.folderType !== '2'">
              <!-- ETL任务新增文件夹按钮 -->
              <span v-if="data.id==='0' || data.id === '1' || data.id === '3'">
                <!-- 新增文件夹 -->
                <addFolder @getEtlTree="getEtlTree"
                           :folderType="data.id"></addFolder>
              </span>
              <!-- 其他情况 -->
              <span v-else>
                <!-- 文件夹 -->
                <span v-if="data.type!=='1' && data.type!=='3' && data.type !== '2' && data.type !== '4'">
                  <!-- 新增任务 -->
                  <addMisson :showDataValidation="showDataValidation"
                             ref="addMission"
                             @getShowMission="getShowMission"
                             @getEtlTree="getEtlTree"
                             :dirId="data.id"></addMisson>
                  <!-- 编辑按钮 -->
                  <editFolder :folderId="data.id"
                              @getEtlTree="getEtlTree"></editFolder>
                  <!-- 删除按钮 -->
                ...
                <span v-if="data.type==='1' || data.type === '3' || data.type === '2' || data.type==='4'">
                  <div id="perTreeMenu"
                       ...
                       :style="{...rightMenu}">
                    <ul>
                      <li @click="cloneMission(data)"><i class="el-icon-tickets"></i> 克隆任务</li>
                    </ul>
                  </div>
                  <a @click="showDrawer(data.id,data)"
                     v-if="data.isAuth !== 0">
                    <ta-icon type="edit"
                             title="编辑任务" />
                  </a>
                  <editMission @getMissionList="getMissionList"
                               ref="editMission"
                               :jobFlowId="data.id"
                               @getEtlTree="getEtlTree"></editMission>
                  <!-- 删除按钮 -->
                  <ta-popconfirm title="确定删除该任务吗？"
                                 @confirm="deleteMission(data.id,data)"
                                 v-if="data.isAuth !== 0">
                    <a>
                      <ta-icon type="delete"
                               title="删除任务" />
              ...
              <span v-if="data.type==='1' || data.type==='3' || data.type === '2' || data.type === '4'">
                ...
                <ta-modal title="文件夹已被删除，请重新选择"
                  ...
                  <ta-select style="width:300px"
                             v-model="folderId"
                             :options="folderList"
                             :optionsKey="optionsKeys"></ta-select>
                </ta-modal>

                <!--彻底删除-->
                <ta-popconfirm title="确定彻底删除该任务吗？"
                               @confirm="deleteMissionTrue(data.id,data)">
                  <a>
                    <ta-icon type="delete"
                             title="彻底删除任务" />
                  </a>
