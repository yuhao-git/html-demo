<template>
  <div class="container">
    <div class="item"
         v-for="item in circlePoints1"
         :style="{ top: `${item.value[0]}px`, left: `${item.value[1]}px` }"></div>
    <div :style="getPositonStyle"
         class="ball"></div>

    <div :style="{ top: `${circlePoints1[(ballIndex + 9) % 36].value[0]}px`, left: `${circlePoints1[(ballIndex + 9) % 36].value[1]}px` }"
         class="ball"></div>

    <div :style="{ top: `${circlePoints1[(ballIndex + 18) % 36].value[0]}px`, left: `${circlePoints1[(ballIndex + 18) % 36].value[1]}px` }"
         class="ball"></div>

    <div :style="{ top: `${circlePoints1[(ballIndex + 27) % 36].value[0]}px`, left: `${circlePoints1[(ballIndex + 27) % 36].value[1]}px` }"
         class="ball"></div>

    <div class="item"
         v-for="item in circlePoints2"
         :style="{ top: `${item.value[0]}px`, left: `${item.value[1]}px` }"></div>
    <!-- 
...
function getCPoint({
  ...
  const data = [];
  for (let index = startAngle; index < endAngle; index = index + p) {
    // 旋转
    let x =
      a *
        Math.cos((Math.PI * 2 * (index - startAngle)) / 360) *
        Math.cos(angle) -
      b *
        Math.sin((Math.PI * 2 * (index - startAngle)) / 360) *
        Math.sin(angle);
    let y =
      a *
        Math.cos((Math.PI * 2 * (index - startAngle)) / 360) *
      ...
      b *
        Math.sin((Math.PI * 2 * (index - startAngle)) / 360) *
        Math.cos(angle);
    data.push(...[{ value: [x + cx, y + cy] }]);
  }
  return data;
}

let circlePoints1 = getCPoint({
  a: 50,
  b: 200,
  p: 10,
  cx: 200,
