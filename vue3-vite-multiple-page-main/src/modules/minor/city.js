...
import { makeCycleTextSprite } from './utils/common.js';

import Animations from './utils/animations.js';

let cityModel = './model/city.fbx';
...
let state = {
  ...
  loadingProcess: 0,
  interactablePoints: [
    { key: '1', value: '图书馆', location: { x: -2, y: 5, z: 0 } },
    { key: '2', value: '教室', location: { x: -4, y: 5, z: 3 } },
    { key: '3', value: '宿舍', location: { x: 4, y: 5, z: 3 } },
  ],
...
export function initThree() {
  ...
  function init() {
    ...
    loader.load(cityModel, mesh => {
      ...
      // 添加交互点
      state.interactablePoints.map(item => {
      ...
      // state.setState({ loadingProcess: Math.floor(res.loaded / res.total * 100) });
    }, err => {
      console.log(err);
    });
  ...
  function handleMouseClick(event) {
    ...
    var intersects = state.raycaster.intersectObjects(interactableMeshes);
    console.log(intersects)
    interactableMeshes.map(item => {
      item.remove(state.billboardLabel);
    })
    if (intersects.length > 0) {
      ...
      Animations.animateCamera(camera, controls, { x: mesh.position.x, y: mesh.position.y + 4, z: mesh.position.z + 12 }, { x: 0, y: 0, z: 0 }, 1200, () => {
        ...
        state.billboardLabel = billboardLabel;
  ...
  function handleMouseEnter(event) {
    ...
    state.mouse.y = - (event.clientY / window.innerHeight) * 2 + 1;
    state.raycaster.setFromCamera(state.mouse, camera);
    var intersects = state.raycaster.intersectObjects(interactableMeshes, true);
    if (intersects.length > 0) {
      let mesh = intersects[0].object
