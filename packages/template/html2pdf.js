...
/**
 * 生成pdf(A4多页pdf截断问题， 包括页眉、页脚 和 上下左右留空)
 * @param {Object} param
...
export async function outputPDF({ element, contentWidth = 550,
  ...
  // addLoading()
  const domOffsetTop = element.offsetTop
  ...
  function removwHeaderDom() {
    headerDom.remove()
  }


  function addFooterDom() {
    footerDom = document.createElement('div')
    footerDom.style.cssText = `
    text-align:center;
    line-height: 4;
    font-weight: 600;
    font-size: 14px;
  ...
  // 添加页眉
  async function addHeader(pdf, contentWidth) {
    const { height: headerHeight, data: headerData, width: hWidth } = await toCanvas(headerDom, contentWidth);
    pdf.addImage(headerData, 'JPEG', 0, 0, contentWidth, headerHeight);
  }

  // 添加页脚
  async function addFooter(pageNum, now, pdf, contentWidth) {
    footerDom.innerText = `第 ${now} 页 / 共 ${pageNum} 页`
    const { height: footerHeight, data: footerData, width: fWidth } = await toCanvas(footerDom, contentWidth);
  ...
  // const pages = [getElementTop(element)];
  const pages = [0];
  // 获取元素距离网页顶部的距离
  ...
  function traversingNodes(nodes) {
    for (let i = 0; i < nodes.length; ++i) {
      ...
      // 图片元素不需要继续深入，作为深度终点
      let list = ['H1', 'H2', "H3", "H4", "SPAN", "P", "CANVAS", "IMG"]
      const isIMG = list.includes(one.tagName);
      // table的每一行元素也是深度终点
      const isTableCol = one.classList?.contains('vxe-table');
      // 特殊的富文本元素
  ...
  for (let i = 0; i < pages.length; ++i) {
    // console.log(`共${pages.length}页， 生成第${i + 1}页`)
    // 根据分页位置新增图片
    ...
    // 添加页眉
    await addHeader(pdf, A4_WIDTH)
    // 添加页脚
    await addFooter(pages.length, i + 1, pdf, A4_WIDTH);
...
// 使用 printOut.print('月缴额变化分析报告', 'printDiv')