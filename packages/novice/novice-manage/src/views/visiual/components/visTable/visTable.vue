  ...
  <div class="dv-scroll-board"
    ...
    <div class="header"
      ...
      <div class="header-item"
           ...
           :key="`${headerItem}${i}`"
           :style="`height: ${mergedConfig.headerHeight}px; line-height: ${mergedConfig.headerHeight}px;width: ${widths[i]}px;background-color: ${mergedConfig.headerBGC};`"
           :align="aligns[i]"
           v-html="headerItem" />
    </div>
    ...
    <div class="rows"
      ...
      <div class="row-item"
           ...
           :style="`height: ${1/rows.length*100}%;line-height: ${1/rows.length*rowHeight}px;`">
        <div class="ceil"
             v-for="(ceil, ci) in row.ceils"
             :key="`${ceil}${ri}${ci}`"
             :style="`width: ${widths[ci]}px;background-color: ${
...
import { deepMerge, deepClone } from "../../js/util";
...
export default {
  ...
  setup() {
    const rowsElement = ref(null);
    // 使用computed来获取ref绑定的元素内容
    const rowHeight = computed(() => {
      return rowsElement.value?.clientHeight;
    });
  ...
  methods: {
    ...
    async animation(start = false) {
      if (!this.mergedConfig.scroll) return;
