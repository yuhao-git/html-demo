...
export default {
  ...
  methods: {
    initChart() {
      ...
      const dataList = [
        { name: "2017", value1: 0, value2: 0,value3: 808,value4: 1000 },
        { name: "2018", value1: 20, value2: 30,value3: 90,value4: 29 },
        { name: "2019", value1: 30, value2: 40,value3: 90,value4: 29 },
        { name: "2020", value1: 40, value2: 50,value3: 90,value4: 29 },
        { name: "2021", value1: 50, value2: 60,value3: 90,value4: 29 },
      ];
      // 提取所有 value 字段（动态）
      ...
      const accHeights = valueDatas.reduce((acc, cur) => {
        ...
        acc.push(cur.map((v, i) => v + last[i] + (acc.length === 0 ? 0 : gapHeight)));
        return acc;
      }, []);

      // 生成主柱子（去除透明 border，改用透明柱体形成间隙）
      const bars = valueKeys.map((key, idx) => ({
        ...
        ...baseBar,
        itemStyle: {
          // 移除 borderColor 与 borderWidth
          color: idx % 2 === 0 ? colors.topGradient : colors.bottomGradient,
        },
      }));

      // 生成透明柱体，形成柱体之间的间隙
      const gapBars = valueKeys.map((key, idx) => ({
        name: `${key}_gap`,
        type: "bar",
        stack: "zs",
        barWidth: 40,
        data: dataList.map(() => gapHeight),
      ...
      valueKeys.forEach((_, idx) => {
        const color = idx % 2 === 0 ? colors.bottomSolid : colors.topSolid;
        // 底部帽子
        caps.push({
